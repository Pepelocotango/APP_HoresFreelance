...
interface ConcepteEditorProps {
  key?: any;
  concepte: Concepte;
  idx: number;
  diaState: Dia;
  setDiaState: Dispatch<SetStateAction<Dia | null>>;
...
const ConcepteEditor = ({ 
  ...
  const updateConcepte = (updates: Partial<Concepte>) => {
    ...
    // Si actualitzem camps que requereixen escriptura/teclat, activem el typing state per posposar l'auto-save
    if ('nom' in updates || 'despesesNotes' in updates || 'despeses' in updates || 'importFix' in updates) {
      triggerTyping();
    }
  };
...
export default function RegistreScreen() {
  ...
  const triggerTyping = () => {
    ...
    typingTimeoutRef.current = setTimeout(() => {
      setIsTyping(false);
    }, 1500); // 1.5s sense teclejar abans de permetre auto-save
  };
  ...
  // Auto-save with debounce (only when not typing)
  useEffect(() => {
    if (saveTimeoutRef.current) {
      clearTimeout(saveTimeoutRef.current);
    }

    if (hasUnsavedChanges && diaState && !isTyping) {
  ...
  }, [diaState, hasUnsavedChanges, isTyping, saveDia]);
  ...
  const handleSave = () => {
    ...
    if (checkOverlaps()) {
      return alert(t('solapament_error'));
    }
    saveDia(diaState);
    setHasUnsavedChanges(false);
    navigate(-1);
